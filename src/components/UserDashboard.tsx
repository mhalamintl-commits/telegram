...
export default function UserDashboard({ user, onUpdateUser, onLogout }: UserDashboardProps) {
  ...
  // Handle Onboarding Stepper logic
  useEffect(() => {
    if (forwarders && forwarders.length === 0) {
      const skipped = localStorage.getItem('teleflow_onboarding_skipped');
      if (!skipped) {
  ...
  const handleSkipOnboarding = () => {
    localStorage.setItem('teleflow_onboarding_skipped', 'true');
    setShowOnboarding(false);
  };
    ...
    <>
      <div className="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
        ...
        <div className="relative overflow-hidden rounded-xl border border-[#1e2230] bg-[#0d0e12] p-4 text-white shadow-md">
          ...
          <div className="mt-2 flex items-baseline gap-2">
            <motion.span 
              key={forwarders.reduce((sum, f) => sum + (f.totalForwarded || 0), 0)}
              initial={{ y: -10, opacity: 0, color: '#fbbf24' }} 
      ...
      {activeSubTab === 'forwarders' && (
        <div className="space-y-6">
          ...
          {/* VISUAL PIPELINE CANVAS BLOCK */}
          {forwarders.length > 0 && (
            <div className="mb-6 grid grid-cols-1 xl:grid-cols-2 gap-6">
              <VisualArchitectureCanvas forwarders={forwarders} logs={logs} />
          ...
          {forwarders.length === 0 ? (
            ...
            <div className="grid grid-cols-1 gap-6 lg:grid-cols-3">
              ...
              <div className="space-y-4">
                <RecentActivityFeed 
                  logs={logs} 
                  onReload={() => { fetchLogs(); fetchForwarders(); }} 
      ...
      {activeSubTab === 'telegram' && (
        <ConnectTelegramAccount user={user} onUpdateUser={onUpdateUser} />
      )}
      ...
      {showOnboarding && (
        <OnboardingStepper 
          isTelegramConnected={user.telegramClient?.status === 'connected'}
          goToTelegramTab={() => setActiveSubTab('telegram')}
          openNewForwarderModal={openNewForwarderModal}
